...
import { Tutor, Room, query, getCoursePrices } from './core';
...
/* Constants */
const levelRegex = /\s*(?:Pre)?[\s-]+[a-z]\d\d?(?:\s?-\s?[a-z]\d\d?)?\s*/i,
		customLevels = [
...
function findLessons(){
	...
	post_data = {
		Name: "",
		sDate: Util.formatDate(options.start),
		Tutor: "",
		shhh: 0,
	...
	if(!_lessons[hash]){
		_lessons[hash] = Promise.all([
				query('process_getCalendarData.php', post_data),
				Room.all()
				...
				data.CalendarCourse.forEach(function(item){
					var start = new Date(item.ScheduleDate),
						...
						tutor = Tutor.find(item.Tutor, true),
				...
				data.CalendarStudent.forEach(function(item){
					...
					attendance = addAttendance({
						...
						original: {
							...
							tutor: Tutor.find(item.ab_tutor)
						}
					});

					Subscription.find({course: lesson.course, student: student})
...
function saveLesson(lesson){
	var startHour = lesson.start.getHours(),
		...
		post_data = {
			...
			sdate: Util.formatDate(lesson.start),
	...
	return query('process_updateCourseSchedule.php',post_data)
		.then(function(data){
 ...
 *
 * @method get
 * @param id {number} ID of course to get
...
function findCourses(options){
	...
	if(!_courses[hash]){
		...
		_courses[hash] = Promise.all([
				query('process_getCourseList.php', post_data),
				Room.all()
			]).then(function(results){
				var data = results[0],
					out = [];
				...
				data.courseschedule.forEach(function(item){
					var start = new Date(item.ScheduleDate),
						...
						tutor = Tutor.find(item.tutorname, true),
						courseID = item.CourseID,
						lessonID = item.CourseScheduleID,
...
function courseFetch(course){
	...
	if(!_courseDetails[course.id]){
		_courseDetails[course.id] = Promise.all([
				query("process_getCourseDetail.php", { courseID: course.id }),
				Room.all()
			])
			.then(function(results){
				var data = results[0],
				...
				tutor = Tutor.get(details.TutorMemberID);
				...
				addCourse({
					...
					room: Room.get(details.DefaultClassroomID),
				...
				data.coursedetailschedule.forEach(function(item){
					...
					lesson = addLesson({
						...
						room: Room.get(item.ClassroomID),
 ...
 * @param {object}  options.lesson Lesson to fetch attendances for
 * @param {object}  [options.student] Specific student to check for in combination with lesson
 * @param {boolean} [options.fetch]  Also fetch full details from the server (to include courseMemberID)
 * @param {boolean} [options.clearCache] Force fetch
 * @return {Promise}
 */
function findAttendances(options){
	if(options.lesson){
		...
		if(!_attendees[id]){
			_attendees[id] = query("process_getCourseScheduleStudents.php", { scheduleID: lesson.id })
				.then(function(data){
...
function saveAttendance(attendance){
	...
	return query("process_updateStudentAttendance.php", {
...
function _parseCourseTitle(course){
	const title = course.originalTitle || course.title || "";
...
function _setCoursePrice(course) {
	var coursePrices = getCoursePrices(),
			prices;
